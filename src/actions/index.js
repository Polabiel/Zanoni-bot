...
Erro: ${error.message}`),
	...
	async sticker() {
		...
		if (!this.isImage && this.isVideo) {
			await this.bot.sendMessage(this.remoteJid, {
				text: errorMessage("Você precisa enviar uma imagem! (no momento não funciona videos)"),
			});
			await this.bot.sendMessage(this.remoteJid, this.checkWarning);
			return;
		}
		...
		if (this.isImage) {
			...
			await this.bot.sendMessage(this.remoteJid,{text:`${errorMessage(`O ${BOT_NAME} n]ao tem suporte para stickers animados`)}`})
			return await this.bot.sendMessage(this.remoteJid, this.checkRed);
		}
		 /*  else {
